...
CREATE TABLE `__new_license_keys` (
	...
	`created_at` integer DEFAULT '"2026-01-26T20:19:44.935Z"' NOT NULL,
	`updated_at` integer DEFAULT '"2026-01-26T20:19:44.935Z"' NOT NULL,
	`expires_at` integer,
	`revoked_at` integer
);
...
CREATE UNIQUE INDEX `machine_id_idx` ON `license_keys` (`machine_id`);--> statement-breakpoint
CREATE TABLE `__new_admin_users` (
	...
	`created_at` integer DEFAULT '"2026-01-26T20:19:44.936Z"' NOT NULL,
...
CREATE TABLE `__new_api_keys` (
	...
	FOREIGN KEY (`revoked_by`) REFERENCES `admin_users`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
INSERT INTO `__new_api_keys`("id", "key", "name", "created_at", "last_used_at", "last_ip_address", "is_active", "usage_count", "revoked_at", "revoked_by", "revocation_reason") SELECT "id", "key", "name", "created_at", "last_used_at", "last_ip_address", "is_active", "usage_count", "revoked_at", "revoked_by", "revocation_reason" FROM `api_keys`;--> statement-breakpoint
...
CREATE TABLE `__new_license_status_logs` (
	...
	`reason` text,
	`timestamp` integer DEFAULT '"2026-01-26T20:19:44.936Z"' NOT NULL,
...
CREATE TABLE `__new_user_submissions` (
	...
	`submission_date` integer DEFAULT '"2026-01-26T20:19:44.935Z"' NOT NULL,
	`ip_address` text,
...
CREATE TABLE `__new_users` (
	...
	`name` text NOT NULL,
	`email` text NOT NULL,
	`created_at` integer DEFAULT '"2026-01-26T20:19:44.934Z"' NOT NULL
);
--> statement-breakpoint
